...
import {
  ArrowRight, Droplets, Package, ShoppingBag, Building2, Home, Store,
  CheckCircle, Truck, Clock, Shield, Star, ChevronRight, Phone, MapPin, TrendingUp
} from 'lucide-react'
...
const segments = [
  { id: 'household', label: 'Household', icon: Home, desc: 'Home delivery for families', color: 'bg-blue-50 text-blue-600 border-blue-200' },
  { id: 'retail', label: 'Retail', icon: Store, desc: 'Shops, kiosks & resellers', color: 'bg-purple-50 text-purple-600 border-purple-200' },
  { id: 'wholesale', label: 'Wholesale', icon: Package, desc: 'Bulk orders, best rates', color: 'bg-water-50 text-water-600 border-water-200', featured: true },
  { id: 'corporate', label: 'Corporate', icon: Building2, desc: 'Office & company supply', color: 'bg-orange-50 text-orange-600 border-orange-200' },
]
...
const testimonials = [
  { name: 'Kwame Mensah', role: 'Wholesale buyer, Tema', text: 'Chico Water has been our water supplier for 2 years. Consistent quality, on-time delivery, no complaints.', stars: 5 },
  { name: 'Abena Owusu', role: 'Household, Osu Accra', text: "I order every week. The app makes it so easy and the delivery guys are always professional.", stars: 5 },
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative min-h-screen bg-gradient-to-br from-water-900 via-water-800 to-water-600 flex items-center overflow-hidden">
        ...
        <div className="absolute top-1/2 left-1/2 w-[600px] h-[600px] -translate-x-1/2 -translate-y-1/2 border border-white/5 rounded-full pointer-events-none" />
        <div className="absolute top-1/2 left-1/2 w-[800px] h-[800px] -translate-x-1/2 -translate-y-1/2 border border-white/5 rounded-full pointer-events-none" />
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <h1 className="text-5xl lg:text-6xl xl:text-7xl font-black text-white leading-[1.05] tracking-tight mb-6 animate-fade-up">
                {s.home_hero_title || 'Pure water, anywhere in Ghana.'}
              </h1>

              <p className="text-white/70 text-lg lg:text-xl leading-relaxed mb-10 max-w-lg animate-fade-up animate-delay-100">
              ...
              <div className="mt-10 flex items-center gap-6 animate-fade-up animate-delay-300">
                <div className="flex -space-x-2">
                  {['KM','AO','EK','BT'].map((init, i) => (
                    <div key={i} className="w-9 h-9 rounded-full bg-gradient-to-br from-water-400 to-water-600 border-2 border-white flex items-center justify-center text-white text-xs font-bold">
                      {init}
                    </div>
                  ))}
                </div>
                <div>
                  ...
                  <p className="text-white/60 text-sm">Trusted by 12,000+ customers</p>
                </div>
              </div>
            </div>
              ...
              <div className="relative">
                ...
                <div className="relative bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-8 animate-float">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="bg-green-400/20 text-green-300 text-xs font-semibold px-3 py-1.5 rounded-full border border-green-400/30">
                      In Transit
                    </div>
                  </div>
                  <div className="space-y-3 mb-6">
                    {[
                      { label: 'Customer', value: 'Abena Owusu' },
                      { label: 'Items', value: '24× 500ml + 3 bags sachet' },
                      { label: 'Total', value: 'GH₵ 99.00' },
                      { label: 'ETA', value: 'Today, 3:30 PM' },
                    ].map(r => (
                  ...
                  <div className="space-y-2">
                    <p className="text-white/60 text-xs font-medium uppercase tracking-wider">Delivery Progress</p>
                    <div className="flex items-center gap-1">
                      {['Confirmed','Packed','In Transit','Delivered'].map((step, i) => (
                        <div key={step} className="flex items-center flex-1">
                          <div className={`h-2 flex-1 rounded-full ${i < 3 ? 'bg-water-400' : 'bg-white/20'}`} />
                          {i < 3 && <div className={`w-2 h-2 rounded-full mx-0.5 ${i < 2 ? 'bg-white' : 'bg-water-300'}`} />}
                        </div>
                      ))}
                    </div>
                    <div className="flex justify-between">
                      {['Confirmed','Packed','Transit','Done'].map((s,i) => (
                        <span key={s} className={`text-xs ${i < 3 ? 'text-white/70' : 'text-white/30'}`}>{s}</span>
                      ))}
                    </div>
                  </div>
                  <div className="mt-6 pt-6 border-t border-white/10 flex items-center gap-3">
                    <div className="w-9 h-9 bg-water-500 rounded-full flex items-center justify-center">
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            ...
            <p className="text-gray-500 text-lg max-w-xl mx-auto">Tailored pricing and service for every type of customer — from a single household to a wholesale distributor.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {segments.map((seg, i) => (
              ...
              >
                {seg.featured && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-water-600 text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider">Best Rates</div>
                )}
                <div className={`w-12 h-12 rounded-xl border-2 ${seg.color} flex items-center justify-center mb-4`}>
              ...
              <div key={product.id} className="card overflow-hidden group hover:-translate-y-1 transition-all duration-300">
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Link href={`/order?product=${product.id}`} className="bg-water-600 hover:bg-water-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-colors">
                      Order →
                    </Link>
                  </div>
                </div>
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            ...
            <p className="text-gray-500 text-lg max-w-xl mx-auto">We built this service from the ground up for Ghanaian homes and businesses.</p>
          </div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-8 sm:p-10">
            <div className="flex flex-col sm:flex-row items-center gap-6">
              <div className="w-16 h-16 bg-water-600 rounded-2xl flex items-center justify-center shrink-0">
      ...
      <section className="py-24 bg-water-900">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            <div className="inline-flex items-center gap-2 bg-white/10 text-white/80 text-xs font-semibold px-3 py-1.5 rounded-full uppercase tracking-wider mb-4">Customer Reviews</div>
      ...
      <section className="py-24 bg-gradient-to-r from-water-600 to-water-800">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <a href={`https://wa.me/${s.business_whatsapp || "233200000000"}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 bg-white/10 border border-white/30 text-white font-semibold px-10 py-4 rounded-2xl text-lg hover:bg-white/20 transition-all">
