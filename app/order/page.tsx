...
import { formatCurrency, GHANA_REGIONS, generateOrderNumber } from '@/lib/utils'
import { useSettings } from '@/components/SettingsProvider'
import { CustomerSegment, OrderItem, PaymentMethod } from '@/types'
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-lg mx-auto px-4 py-32 text-center">
        <div className="card p-10">
          ...
          <p className="text-gray-500 mb-6">Your order has been confirmed. You'll receive an SMS shortly.</p>
            ...
            {STEPS.map((s, i) => (
              <div key={s} className="flex items-center gap-2">
                <div className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold transition-all ${
                  i === step ? 'bg-white text-water-700' :
                  i < step ? 'bg-water-500/50 text-white' : 'bg-white/10 text-white/50'
                }`}>
                  <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold ${
                    i < step ? 'bg-green-400 text-white' : i === step ? 'bg-water-600 text-white' : 'bg-white/20 text-white/50'
                  }`}>
                    {i < step ? '✓' : i + 1}
                  </span>
                  {s}
                  ...
                  {segments.map(s => (
                    <button
                      ...
                      className={`p-5 rounded-2xl border-2 text-left transition-all ${
                        segment === s.id ? 'border-water-600 bg-water-50' : 'border-gray-100 hover:border-gray-200 bg-white'
                      }`}
                    >
                      <div className="flex items-center justify-between mb-2">
                        <span className="font-bold text-gray-900">{s.label}</span>
                        {segment === s.id && <CheckCircle className="w-5 h-5 text-water-600" />}
                      </div>
            ...
            {step === 1 && (
              <div className="space-y-4">
                <div className="card p-6">
                  ...
                  <p className="text-gray-500 text-sm mb-6">Prices shown for <span className="font-semibold text-water-600 capitalize">{segment}</span> customers.</p>
                  <div className="space-y-3">
                      ...
                      return (
                        <div key={product.id} className={`flex items-center gap-4 p-4 rounded-2xl border-2 transition-all ${qty > 0 ? 'border-water-300 bg-water-50' : 'border-gray-100 bg-gray-50'}`}>
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            <button onClick={() => removeFromCart(product.id)} className="w-8 h-8 rounded-full bg-white border border-gray-200 flex items-center justify-center hover:bg-gray-50 transition-colors">
                              <Minus className="w-3 h-3 text-gray-600" />
                            </button>
                            <span className="w-8 text-center font-bold text-sm">{qty}</span>
                            <button onClick={() => addToCart(product.id)} className="w-8 h-8 rounded-full bg-water-600 flex items-center justify-center hover:bg-water-700 transition-colors">
              ...
              <div className="card p-8">
                ...
                <div className="grid gap-5">
                  ...
                  <div>
                    ...
                    <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} className="input resize-none" rows={3} placeholder="e.g. Leave with security guard, call on arrival, etc." />
                  </div>
                </div>
              </div>
            )}
            ...
            {step === 3 && (
              <div className="card p-8">
                ...
                <div className="space-y-3 mb-8">
                  {[
                    { id: 'momo', label: 'Mobile Money', desc: 'MTN, Vodafone, AirtelTigo', icon: Smartphone },
                    { id: 'card', label: 'Debit / Credit Card', desc: 'Visa, Mastercard', icon: CreditCard },
                    { id: 'cash', label: 'Cash on Delivery', desc: 'Pay when your order arrives', icon: Banknote },
                  ].map(m => (
                    <button
                      key={m.id}
                      onClick={() => setPayment(m.id as PaymentMethod)}
                      className={`w-full flex items-center gap-4 p-5 rounded-2xl border-2 text-left transition-all ${
                        payment === m.id ? 'border-water-600 bg-water-50' : 'border-gray-100 hover:border-gray-200'
                      }`}
                    >
                ...
                {payment === 'momo' && (
                  <div>
                    ...
                    <div className="flex gap-3">
                      {[{ id: 'mtn', label: 'MTN', color: 'bg-yellow-400' }, { id: 'vodafone', label: 'Vodafone', color: 'bg-red-500' }, { id: 'airteltigo', label: 'AirtelTigo', color: 'bg-blue-500' }].map(n => (
                        <button
                          key={n.id}
                    ...
                    <div className="mt-4">
                      ...
                      <input className="input" placeholder="024 000 0000" defaultValue={form.phone} />
                    </div>
                  </div>
                )}
              </div>
            ...
            {step === 4 && (
              <div className="card p-8">
                ...
                <div className="bg-gray-50 rounded-xl p-5 space-y-3 mb-8">
                  <div className="flex justify-between text-sm"><span className="text-gray-500">Delivery to</span><span className="font-medium text-right max-w-[200px]">{form.address}, {form.region}</span></div>
                ...
                >
                  {loading ? (
                    ...
                    <><ShoppingCart className="w-5 h-5" /> Place Order · {formatCurrency(total)}</>
                  )}
                </button>
              </div>
            )}
          ...
          <div className="lg:col-span-1">
            <div className="card p-6 sticky top-24">
              <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
                <ShoppingCart className="w-5 h-5 text-water-600" />
                Your cart
                  ...
                  {cartItems.map(item => (
                    <div key={item.product_id} className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => deleteFromCart(item.product_id)} className="text-gray-300 hover:text-red-400 transition-colors">
