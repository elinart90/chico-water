        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          <div className="lg:col-span-1">
            <Link href="/" className="flex items-center gap-2.5 mb-4">
              ...
              <div>
                <div className="text-white font-bold text-base">{s.business_name || 'Chico Water'}</div>
                <div className="text-water-400 text-[10px] font-medium uppercase tracking-widest">Limited Company</div>
              </div>
            </Link>
            <p className="text-sm leading-relaxed text-gray-500 mb-5">
              {s.business_tagline_2 || "Ghana's premier water supplier."}
            </p>
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {[
                ...
                <li key={link.href}>
                  <Link href={link.href} className="text-sm hover:text-water-400 transition-colors">{link.label}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3">
              <li className="flex items-start gap-3">
                <MapPin className="w-4 h-4 text-water-400 mt-0.5 shrink-0" />
                <span className="text-sm">{s.business_address || 'Industrial Area, Accra, Ghana'}</span>
              </li>
              <li className="flex items-center gap-3">
                <Phone className="w-4 h-4 text-water-400 shrink-0" />
                <a href={`tel:${s.business_phone}`} className="text-sm hover:text-water-400 transition-colors">
                  {s.business_phone || '+233200000000'}
                </a>
              </li>
              <li className="flex items-center gap-3">
                <Mail className="w-4 h-4 text-water-400 shrink-0" />
